...
class FireworksProvider(AIProviderBase):
    """
    Fireworks API провайдер.
    ...
    def __init__(self, api_key: Optional[str] = None, model: Optional[str] = None):
        """
        Инициализация Fireworks провайдера.

        Args:
            api_key: Fireworks API ключ (по умолчанию из FIREWORKS_API_KEY)
            model: Название модели (по умолчанию llama-v3p1-70b-instruct)
        """
    ...
    async def generate(self, prompt: str, **kwargs) -> str:
        """
        Генерация ответа через Fireworks API.
        ...
        """
        if not self.api_key:
            raise ValueError("Fireworks API ключ обязателен")
        ...
        async with httpx.AsyncClient(timeout=self.timeout) as client:
            try:
                ...
                if "choices" in result and len(result["choices"]) > 0:
                    ...
                    logger.error(f"Неожиданный формат ответа Fireworks: {sanitize_error_message(str(result))}")
                    raise ValueError("Некорректный формат ответа от Fireworks")

            except httpx.HTTPError as e:
                logger.error(f"Ошибка Fireworks API: {sanitize_error_message(e)}")
                raise

    async def health_check(self) -> bool:
        """
        Проверка доступности Fireworks API.

        Returns:
            True если API доступен, False иначе
        """
        if not self.api_key:
            logger.warning("Fireworks API ключ не настроен")
            return False
        ...
        async with httpx.AsyncClient(timeout=10.0) as client:
            try:
                ...
                logger.error(f"Fireworks health check не прошёл: {sanitize_error_message(e)}")
                return False
    ...
    def _supports_response_format(self) -> bool:
        ...
        F011-B: Fireworks support for response_format is TBD.
