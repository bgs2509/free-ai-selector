...
class GitHubModelsProvider(AIProviderBase):
    ...
    def __init__(self, api_key: Optional[str] = None, model: Optional[str] = None):
        """
        Инициализация GitHub Models провайдера.

        Args:
            api_key: GitHub Personal Access Token (по умолчанию из GITHUB_TOKEN)
    ...
    async def generate(self, prompt: str, **kwargs) -> str:
        """
        Генерация ответа через GitHub Models API.
        ...
        async with httpx.AsyncClient(timeout=self.timeout) as client:
            try:
                ...
                if "choices" in result and len(result["choices"]) > 0:
                    ...
                    logger.error(f"Неожиданный формат ответа GitHub Models: {sanitize_error_message(str(result))}")
                    raise ValueError("Некорректный формат ответа от GitHub Models")

            except httpx.HTTPError as e:
                logger.error(f"Ошибка GitHub Models API: {sanitize_error_message(e)}")
                raise

    async def health_check(self) -> bool:
        """
        Проверка доступности GitHub Models API.

        Returns:
            True если API доступен, False иначе
        """
        if not self.api_key:
            logger.warning("GitHub Token не настроен")
            return False

        headers = {"Authorization": f"Bearer {self.api_key}"}

        # Простой health check через HEAD запрос
        async with httpx.AsyncClient(timeout=10.0) as client:
            try:
                ...
                # Любой ответ кроме 5xx считаем успешным
                return response.status_code < 500

            except Exception as e:
                logger.error(f"GitHub Models health check не прошёл: {sanitize_error_message(e)}")
                return False
    ...
    def _supports_response_format(self) -> bool:
        ...
        F011-B: GitHub Models supports json_schema format.
        """
        return True
